...
import { get_id, decode_json } from '@shared/ts/helpers';

type BackendResponse<D> = any;
...
class Proxies {
            ...
            {},
            {
              get(target, method_name) {
                return async (...args: any[]) => {
                  ...
                  let result = await _this.backend(`${backend_root}/trpc`, new_args);
  ...
  async get_backend_response(endpoint, args?) {
    try {
      ...
      let json = await decode_json(text);
      if (json) {
        return json as any;
      } else {
        return {
