...
import popup_html from '@src/html/popup.html?raw';
import popup_css from '@src/css/popup.css?raw';
import logo_progress_svg from '@src/svg/ripple.svg?raw';
import project_css from '@src/css/project.css?raw';
import content_css from '@src/css/content.css?raw';
import content_isolated_css_raw from '@src/css/content_isolated_compiled.css?raw';
import logo_svg from '@shared/slots/logo-main.svg?raw';
import { clone, html_to_element } from '@shared/ts/helpers';
...
export default class Popup {
  ...
  get_popup_rect_onboarding_small() {
    let width = 520;
  ...
  get_rect_after_bubble() {
    ...
    console.log('get_draggable_window_rect', dwr);
    //
    let rect: any = {};
  ...
  async init() {
    ...
    console.log('ts123', Date.now());
    //
    let style_content = html_to_element(
    ...
    document.documentElement.prepend(style_content);
    //
    ...
    // this.container.querySelector('.chromane-logo').innerHTML = logo_svg;
    // this.container.querySelector('.chromane-logo-progress').innerHTML = logo_progress_svg + logo_svg;

    // www.whisker.com
    // let result = await ctrl_content.proxy_back.project.get_ai_response('What is the best litter box for two cats?', 'www.whisker.com');
    // console_log('ping result', result);
    // console_log('ping result', result);
    //
    // });
    ...
    // }
    //
    const host = html_to_element(`<div class = "chromane-shadow-root-host module-popup" ></div>`) as HTMLElement;
    ...
    let mousemove_listener = (event) => {
      ...
      if (this.dragging_initialized === false) {
        ...
        //
        this.dragging_initial_rect = clone(this.rect);
        //
        this.dragging_initial_mouse_x = event.clientX;
        this.dragging_initial_mouse_y = event.clientY;
        //
      }
      // figure out if mouse was moved or not
      // if it was moved a little bit - don't count that and trigger
      // the click event in the mouseup listener
      let diff_x = this.dragging_initial_mouse_x - event.clientX;
      let diff_y = this.dragging_initial_mouse_y - event.clientY;
      let diff_real = Math.sqrt(diff_x * diff_x + diff_y * diff_y);
      if (diff_real > 7) {
    ...
    let mouseup_listener = (event) => {
      // console_log('mouseup_listener', event);
    ...
    let mousedown_listener = (event) => {
      // console_log('mousedown_listener', event);
      if (event.which !== 1) {
        return;
      }
      //
    ...
    // });
    // this.container.querySelector(`[data-qa="chromane-activate"]`).addEventListener('click', () => {
    //   this.show();
    // });
  }
  ...
  async show_as_bubble() {
    ...
    //
    let rect_hidden: any = {};
    rect_hidden.top = this.storage.popup.rect.top;
    rect_hidden.left = this.get_width() + this.bubble_width + 24;
    rect_hidden.height = this.bubble_height;
    rect_hidden.width = this.bubble_width;
    //
    this.set_transition_status(TransitionStatus.not_active);
    this.rect_set_apply(rect_hidden);
    //
    let new_rect: any = {};
    new_rect.top = this.storage.popup.rect.top;
    new_rect.left = this.get_width() - this.bubble_width - 24;
    ...
    if (this.transform_dimensions.includes('left')) {
      if (
        ...
        if (new_width > this.min_width) {
          ...
          // legacy this was used for resizing host website body
          // use sidePanel api now instead
  ...
  exec_after_mouse_up() {
    ...
    if (
      ...
      draggable_window_rect = clone(this.rect);
    } else {
      draggable_window_rect = clone(this.storage.popup.draggable_window_rect);
    }
    //
    ...
    //
    console.log('draggable_window_rect_storage', draggable_window_rect);
    this.storage_set({
      popup: {
        rect: this.rect,
  ...
  async storage_get() {
    ...
    if (!storage.popup) {
      let default_storage_popup;
      if (window.innerHeight > 640) {
        default_storage_popup = {
  ...
  async show() {
    if (this.status === PopupStatus.hidden) {
      this.set_status(PopupStatus.draggable_window);
    }
  }
  show_fast() {
    this.set_transition_status(TransitionStatus.not_active);
  ...
  toggle() {
    if (this.status === PopupStatus.hidden) {
      this.show();
    } else if (this.status === PopupStatus.draggable_window || this.status === PopupStatus.draggable_overlay) {
      this.hide();
  ...
  get_draggable_window_rect() {
    return clone(this.storage.popup.draggable_window_rect);
  }
  ...
  async set_status(status, info?) {
    //
    console_log('status_current', this.status);
    console_log('status_new', status);
    //
    let new_status;
    let solid_popup_rect = this.get_popup_rect_small();
    let draggable_overlay_rect;
    if (status === 'hide_instant') {
      this.set_transition_status('not-active');
      this.container.style.opacity = 0;
      return;
    }
    if (status === 'show_instant') {
      this.set_transition_status('not-active');
      this.container.style.opacity = 1;
      return;
    } else if (status === 'fullscreen_instant') {
      this.set_transition_status('not-active');
      this.rect_set_apply({
        top: 0,
        left: 0,
      ...
      return;
    } else if (status === 'fullscreen_instant_reset') {
      this.set_transition_status('active');
      let rect = this.get_draggable_window_rect();
      this.rect_set_apply(rect);
      return;
    } else if (status === 'auth_popup') {
    ...
    } else if (status === 'onboarding_popup_small') {
      new_status = PopupStatus.draggable_window;
      this.storage.popup.rect = this.get_popup_rect_onboarding_small();
    } else {
      new_status = status as PopupStatus;
    }
    if (false) {
      // skip
    } else if (this.status === PopupStatus.hidden && new_status === PopupStatus.button) {
      // define target rect
      ...
      this.set_transition_status(TransitionStatus.not_active);
      let rect_real = clone(this.storage.popup.rect);
      let rect_temp = clone(this.storage.popup.rect);
    ...
    } else if (this.status === PopupStatus.hidden && new_status === PopupStatus.draggable_overlay) {
      this.set_transition_status(TransitionStatus.not_active);
      let rect_real = clone(this.storage.popup.rect);
      let rect_temp = clone(this.storage.popup.rect);
      rect_temp.top = rect_real.top - 64;
      ...
      // prepare
      this.set_transition_status(TransitionStatus.not_active);
      let rect = clone(solid_popup_rect);
      if (rect) {
      ...
      this.set_class('chromane-transition-status', 'not-active');
      this.container.offsetHeight;
      this.rect_set_apply({
        left: this.get_width(),
      });
      ...
      this.set_container_mode(ContainerMode.popup);
      this.rect_set_apply(rect);
    } else if (this.status === PopupStatus.bubble && new_status === PopupStatus.draggable_window) {
      this.set_transition_status(TransitionStatus.active);
      let rect = this.get_draggable_window_rect();
      this.rect_set_apply(rect);
      this.status = PopupStatus.draggable_window;
      ...
      let new_rect: any = {};
      new_rect.top = this.storage.popup.rect.top;
      // this.rect.top + this.rect.height / 2 - this.bubble_height / 2;
      new_rect.left = this.get_width() - this.bubble_width - 24;
      new_rect.height = this.bubble_height;
      new_rect.width = this.bubble_width;
      //
    ...
    this.on_status_settled();
    //
  }

  shift_position_if_overflow(
    config: { padding: number; min_width: number; min_height: number } = {
      padding: 0,
      min_height: 100,
      min_width: 100,
    }
  ...
  private async handle_window_resize() {
    return;
    if (this.status === PopupStatus.sidebar_left) {
      this.set_transition_status(TransitionStatus.not_active);
      ...
      let rect = this.shift_position_if_overflow({
        padding: 0,
        min_height: 64,
        min_width: 64,
      });
  ...
  private async on_status_settled() {
    const handle_statuses = [
      PopupStatus.sidebar_left,
      PopupStatus.sidebar_right,
      PopupStatus.bubble,
      PopupStatus.draggable_window,
      PopupStatus.draggable_overlay,
    ];

    if (!handle_statuses.includes(this.status)) {
      return;
    }

    let exec_window_resize = false;

    let w_height = window.innerHeight;
    let w_width = window.innerWidth;
    const r_bottom = this.rect.top + this.rect.height;
    const r_right = this.rect.left + this.rect.width;

    if (this.rect.top < 0) {
      exec_window_resize = true;
    } else if (r_bottom > w_height) {
      exec_window_resize = true;
    } else if (w_width < r_right) {
      exec_window_resize = true;
    } else if (this.rect.left < 0) {
      exec_window_resize = true;
    }

    if (exec_window_resize) {
      this.handle_window_resize();
    }
  }
}
