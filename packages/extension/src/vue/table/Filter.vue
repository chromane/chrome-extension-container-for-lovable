...
import { FBTableColumnConfig, FBTableFilterRule } from '../../../../shared/tables/types';
import { computed, onMounted, reactive } from 'vue';
import { COLUMN_STATIC_CONFIGURATION } from './constants';
import { clone, stop_propagation_smart } from '@shared/ts/helpers';
...
const handlers = {
  handle_click_filter_btn(e: any) {
    ...
    //
    stop_propagation_smart(e);
    //
    if (!is_open) {
      if (props.applied_rules?.length > model.rules?.length) {
        model.rules = clone(props.applied_rules);
      }
...
<template>
  <div class="table-filter">
    <div class="table-filter-btn" :class="{ filtered: props.applied_rules.length > 0 }" @click="handlers.handle_click_filter_btn">
      <div class="table-filter-btn__svg" v-html="mdi_filter"></div>
      <span v-if="props.applied_rules.length > 0">
        <span>Filtered by {{ ' ' }}</span>
        <span v-for="item in props.applied_rules" :key="item.config.name" v-text="item.config.title + ' '"></span>
      </span>
    ...
    >
      <div class="filter-popup__wrap">
        <div class="filter-popup-header">
          ...
          <div class="filter-popup-body" v-if="model.rules?.length > 0">
            <div class="filter-popup-body__filters">
              <div class="filter-popup-rule" v-for="(rule, index) in model.rules" :key="rule.config.name">
                <div class="filter-popup-rule__label">Where</div>
                <div class="filter-popup-rule__actions" v-if="rule.config">
                  <select data-type="field" @change="(e) => handlers.handle_change_select_field(e, index)" :value="rule.config.name">
                    <option
                  ...
                  <select data-type="method" :value="rule.method" @change="(e) => handlers.handle_change_select_method(e, index)">
                    <option
                  ...
                  <div class="filter-popup-rule__enter">
                    <input v-if="rule.method === 'is'" type="text" :value="rule.value" @input="(e) => handlers.handle_change_rule_value(e, index)" />
                  </div>
                  <button class="filter-popup-rule__delete" v-html="mdi_trash" @click="handlers.handle_click_delete(index)"></button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="filter-popup-footer">
            <div class="filter-popup-footer__create" v-if="model.rules.length === 0">
              <button data-action="add" @click="handlers.handle_click_create_rule">
                <div class="svg-wrap" v-html="mdi_plus"></div>
                Add condition
              </button>
            </div>
            <button
